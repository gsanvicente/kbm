...
- Estado: **Diseño completo, implementación pendiente** — misma
  dependencia que `docs/feature/portal-autoservicio-tarjetahabiente/README.md`
  (CRUD de Clientes y de Tarjetahabientes primero).
...
- Amenazas relevantes: `docs/security/threat-model.md` puntos 9 (para
  contraste — este NO es el mismo caso), 11 y 12
...
distintos: fuera de alcance.

## Seguridad
- **Mensaje de error genérico**: nunca se distingue "número con formato
  inválido" de "número válido pero no pertenece a nuestro universo" — ver
  threat-model punto 12.
- **Límite de intentos fallidos** por sesión/usuario, con bloqueo
  temporal — sin esto, el mensaje genérico por sí solo no evita que
  alguien pruebe muchos números buscando una coincidencia.
...
- El hash de PAN vive en un almacén separado y de acceso restringido, no
  mezclado con las lecturas normales de tarjetas — ver la ADR.
